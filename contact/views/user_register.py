...
class AddRegisterUser(APIView):
    def get(self,request):
        ...
        firstpage=paginator.page(1).start_index()
    ...
    def post(self, request, format=None):
        objserializer = UserRegisterSerializer(data=request.data)
        data ={}
        if objserializer.is_valid():
            user =objserializer.save()
            data['response']= 'successfully registered a new user'
            data['email']=user.email
            data['firs_name']=user.first_name
        else:
            data =objserializer.errors
